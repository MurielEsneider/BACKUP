...
CREATE TABLE empleado (
  codigo_empleado INTEGER NOT NULL AUTO_INCREMENT,
...
BEGIN
    ...
    read_loop: LOOP
        FETCH cliente_cursor INTO id_cliente;
        IF done THEN
            LEAVE read_loop;
        END IF;

        -- Calcular el total de pedidos y pagos para el cliente actual
        SET total_pedidos = calcular_suma_pedidos_cliente(id_cliente);
        SET total_pagos = calcular_suma_pagos_cliente(id_cliente);
        SET pendiente = total_pedidos - total_pagos;

        -- Si hay pagos pendientes, insertar en la tabla de resultados
        IF pendiente > 0 THEN
            INSERT INTO clientes_con_pagos_pendientes (id_cliente, suma_total_pedidos, suma_total_pagos, pendiente_de_pago)
            VALUES (id_cliente, total_pedidos, total_pagos, pendiente);
        END IF;
    END LOOP;
...
CREATE FUNCTION cantidad_total_de_productos_vendidos(producto_codigo VARCHAR(15)
RETURNS INT
READS SQL DATA
BEGIN
...
SELECT cantidad_total_de_productos_vendidos(22225);
...
BEGIN
    ...
    -- Limpiar la tabla productos_vendidos
    ...
    read_loop: LOOP
        FETCH producto_cursor INTO codigo_producto;
        IF done THEN
            LEAVE read_loop;
        END IF;

        -- Calcular la cantidad total de productos vendidos usando la función
        SET cantidad_total = cantidad_total_de_productos_vendidos(codigo_producto);

        -- Insertar en la tabla productos_vendidos
        INSERT INTO productos_vendidos (codigo_producto, cantidad_total)
        VALUES (codigo_producto, cantidad_total);
    END LOOP;
...
-- Verificar los datos en la tabla productos_vendidos
SELECT * FROM productos_vendidos;


-- Transacciones Jardineria 

INSERT INTO oficina (codigo_oficina, ciudad, linea_direccion1, telefono, codigo_postal, pais)
VALUES ('OFI001', 'Pasto', 'Carrera 12 Calle 13', '123456789', '232323', 'Colombia');


SELECT codigo_oficina FROM oficina WHERE ciudad = 'Pasto';


-- Inserta un empleado para la oficina a que sea representante de ventas.
INSERT INTO empleado (nombre, apellido1, puesto, codigo_oficina, email, extension)
VALUES ('Juan', 'Lopez', 'Representante de Ventas', 2, 'juan.lopez@ejemplo.com', '000');
