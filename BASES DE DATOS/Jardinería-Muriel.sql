...
-- EJERCICIO6
SELECT DISTINCT oficina.linea_direccion1, oficina.linea_direccion2, oficina.ciudad, oficina.region, oficina.pais
FROM oficina
...
-- EJERCICIO8
SELECT empleado.nombre AS nombre_empleado, empleado.apellido1 AS apellido_empleado, jefe.nombre AS nombre_jefe, jefe.apellido1 AS apellido_jefe
FROM empleado
...
SELECT e1.nombre AS nombre_empleado, 
       e1.apellido1 AS apellido_empleado, 
       e2.nombre AS nombre_jefe, 
       e2.apellido1 AS apellido_jefe, 
       e3.nombre AS nombre_jefe_del_jefe, 
       e3.apellido1 AS apellido_jefe_del_jefe
FROM empleado e1
...
WHERE pedido.fecha_entrega > pedido.fecha_esperada
   OR (pedido.fecha_entrega IS NULL AND CURDATE() > pedido.fecha_esperada);
-- EJERCICIO10
...
LEFT JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente
LEFT JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente
WHERE pago.codigo_cliente IS NULL OR pedido.codigo_pedido IS NULL;
-- EJERCICIO3
...
FROM empleado
LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_cliente
...
-- EJERCICIO6
SELECT empleado.nombre, empleado.apellido1, empleado.apellido2, 
       oficina.ciudad, oficina.pais, oficina.linea_direccion1, oficina.linea_direccion2
FROM empleado
LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas
JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina
WHERE cliente.codigo_cliente IS NULL;
...
-- EJERCICIO7
SELECT nombre, apellido1, apellido2, 'No tiene oficina asociada' AS estado
FROM empleado
WHERE codigo_oficina IS NULL

UNION

SELECT nombre, apellido1, apellido2, 'No tiene cliente asociado' AS estado
FROM empleado
...
-- EJERCICIO8
SELECT producto.nombre, producto.codigo_producto, producto.gama
FROM producto
...
-- EJERICICIO10
SELECT oficina.codigo_oficina, oficina.ciudad, oficina.pais, oficina.linea_direccion1, oficina.linea_direccion2
FROM oficina
...
-- EJERCICIO12
SELECT e.nombre AS nombre_empleado, e.apellido1 AS apellido1_empleado, e.apellido2 AS apellido2_empleado,
       j.nombre AS nombre_jefe, j.apellido1 AS apellido1_jefe, j.apellido2 AS apellido2_jefe
FROM empleado e
...
SELECT 
    ...
    SUM(detalle_pedido.cantidad * detalle_pedido.precio_unidad) AS total_facturado_sin_iva,
...
WHERE 
    detalle_pedido.cantidad * detalle_pedido.precio_unidad * 1.21 > 3000
GROUP BY 
...
-- EJERCICIO4
SELECT codigo_cliente, nombre_cliente, limite_credito, total_pagos
FROM (
    SELECT c.codigo_cliente, c.nombre_cliente, c.limite_credito,
           (SELECT SUM(total) FROM pago WHERE codigo_cliente = c.codigo_cliente) AS total_pagos
    FROM cliente c
) AS cliente_pagos
WHERE limite_credito > total_pagos;
-- EJERCICIO4
